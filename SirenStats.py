...
def process_files(download_dir):
    try:
        # Define file paths
        csv_path = os.path.join(download_dir, "alarms_report.csv")
        xlsx_path = os.path.join(download_dir, "rms_station_report.xlsx")

        # Check if files exist
        if not os.path.exists(csv_path):
            st.error("Banglalink Alarms CSV file not found.")
            return
        if not os.path.exists(xlsx_path):
            st.error("Eye Electronics XLSX file not found.")
            return

        # Read the CSV file
        csv_data = pd.read_csv(csv_path)
        st.success("Banglalink Alarms CSV file read successfully!")

        # Read the XLSX file with header in row 3 (zero-based index 2)
        xlsx_data = pd.read_excel(xlsx_path, header=2)
        st.success("Eye Electronics XLSX file read successfully!")
    ...
    def main():
        ...
        **SirenStats** is a Streamlit application that processes alarm data by merging relevant information
        ...
        st.header("Download Files Automatically")
        st.markdown("""
        **Note:** Automated downloading is handled by a separate script (`download_data.py`). Please ensure that you've run the download script
        and the files are available in the `downloads` directory before proceeding.
        """)

        if st.button("Process Downloaded Files Automatically"):
            download_dir = os.path.join(os.getcwd(), "downloads")
            process_files(download_dir)

        st.header("Upload and Process Files Manually")
        st.markdown("""
        ...
        if st.button("Process Uploaded Files"):
            if csv_file and xlsx_file:
                process_files_from_upload(csv_file, xlsx_file)
            else:
                st.error("Please upload both CSV and XLSX files before processing.")

    def process_files_from_upload(csv_file, xlsx_file):
        try:
            # Read the CSV file
