        ...
        if 'Elapsed Time' in df_previous.columns and 'Zone' in df_previous.columns and 'Tenant' in df_previous.columns:
            ...
            def convert_to_hours(elapsed_time):
                try:
                    ...
                    st.warning(f"Error converting '{elapsed_time}': {e}")
                    return None

            df_previous['Elapsed Time (hours)'] = df_previous['Elapsed Time'].apply(convert_to_hours)

            # Display processed data
            st.write("Previous Outage Data Processed")
            st.table(df_previous[['Tenant', 'Zone', 'Elapsed Time (hours)']])

        else:
            st.error("Required columns missing in the uploaded Previous Outage file.")
    else:
