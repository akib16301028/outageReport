...
if show_client_wise and not regions_zones.empty:
    ...
    st.table(df_default_exploded[['Cluster', 'Zone', 'Clients']])

# Upload Outage Data
uploaded_outage_file = st.file_uploader("Please upload an Outage Excel Data file", type="xlsx", key="outage_uploader")

# Declare reports dictionary for storing outage data reports
reports = {}

# Process outage data but don't display it yet
if uploaded_outage_file:
    ...
    if 'RMS Alarm Elapsed Report' in xl_outage.sheet_names:
        ...
        if 'Site Alias' in df_outage.columns:
            ...
            # Function to generate report (internally in the backend)
            ...
            # Generate reports for each client in the background
...
# Process the previous outage data and merge with outage data
if uploaded_outage_file and uploaded_previous_file:
    if 'generate_report' not in st.session_state:
        st.session_state.generate_report = False

    if st.button("Generate and Merge Reports"):
