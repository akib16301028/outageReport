...
uploaded_previous_file = st.file_uploader("Please upload a Previous Outage Excel Data file", type="xlsx", key="previous")

# Upload Outage Data
st.subheader("Upload Outage Data")
uploaded_outage_file = st.file_uploader("Please upload an Outage Excel Data file", type="xlsx", key="outage")
...
if uploaded_previous_file and uploaded_outage_file:
    ...
    if 'Report Summary' in xl_previous.sheet_names:
        ...
        if 'Elapsed Time' in df_previous.columns and 'Zone' in df_previous.columns and 'Tenant' in df_previous.columns:
            ...
            selected_client_previous = st.selectbox("Select a Client (Tenant)", clients)

            df_filtered = df_previous[df_previous['Tenant'] == selected_client_previous]
            ...
            if not df_filtered.empty:
                ...
                st.error(f"No data available for the client '{selected_client_previous}'")
        else:
            ...
            if 'Site Alias' in df.columns:
                ...
                def generate_report(client_df, selected_client):
                    ...
                    if 'Duration' in report.columns:
                        ...
                        st.warning(f"'Duration (hours)' column is missing for client: {selected_client}")
...
if not regions_zones.empty:
    st.sidebar.subheader("Client Site Count from RMS Station Status Report")
    show_site_count = st.sidebar.checkbox("Show Client Site Count")
    ...
    if show_site_count:
        ...
        try:
            ...
            if 'Site Alias' in df_initial.columns:
                ...
                st.write("Client Site Count from RMS Station Status Report:")
